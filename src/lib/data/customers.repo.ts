...
export const createCustomer = (input: { name: string; phone_e164: string; email?: string | null; notes?: string | null }) =>
  db.table<Customer>('customers').create(input);

export const updateCustomer = (id: string, patch: Partial<Pick<Customer, 'name' | 'phone_e164' | 'email' | 'notes'>>) =>
