...
import { listTickets, assignTicket, resolveTicket } from '@/lib/data/tickets.repo';
import { listCustomers } from '@/lib/data/customers.repo';
import { listTicketNotes, createTicketNote } from '@/lib/data/ticket_notes.repo';
...
export default function TicketDetailScreen() {
  ...
  const whatsappLink = useMemo(() => {
    if (!ticket || !customer?.phone_e164) return null;
    return buildWhatsAppLink(customer.phone_e164, customer.name, ticket.number, ticket.subject);
  }, [ticket, customer]);
  ...
  if (!ticket) {
    return (
      <div className="space-y-4">
        <Button variant="ghost" size="icon" asChild><Link to="/tickets"><ArrowLeft className="h-4 w-4" /></Link></Button>
            ...
            {!isRep && (
              <div className="pt-2 space-y-2">
                ...
                <div className="flex justify-end">
                  <Button size="sm" className="gap-1" onClick={handleAddNote} disabled={savingNote || !newNote.trim()}>
                    {savingNote ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />} Salvar nota
                  </Button>
                </div>
              </div>
            )}
        ...
        <div className="space-y-4">
          {!isRep && (
            <div className="rounded-lg border border-border bg-card p-4 space-y-2">
              ...
              {ticket.status === 'open' && (
                <Button size="sm" variant="outline" className="w-full" onClick={handleAssign} disabled={updatingStatus}>
                  Atender
                </Button>
              )}
              {ticket.status !== 'resolved' && (
                <Button size="sm" variant="outline" className="w-full" onClick={handleResolve} disabled={updatingStatus}>
                  Concluir
                </Button>
              )}
              {whatsappLink && (
                <Button size="sm" variant="outline" className="w-full gap-2 text-sla-ok border-sla-ok/40 hover:bg-sla-ok/10" asChild>
          ...
          {customer && (
            <div className="rounded-lg border border-border bg-card p-4 space-y-1">
              <h2 className="text-sm font-semibold text-foreground mb-2">
                <Link to={`/customers/${customer.id}`} className="hover:text-primary">{customer.name}</Link>
              </h2>
