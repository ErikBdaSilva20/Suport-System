...
import { createCustomer, listCustomers } from '@/lib/data/customers.repo';
...
export default function TicketNewScreen() {
  ...
  const handleSubmit = async () => {
    ...
    const category =
      categorySelection === OTHER_CATEGORY ? customCategory.trim() : categorySelection;
    if (!category) {
        ...
        <div className="space-y-2">
          ...
          {categorySelection === OTHER_CATEGORY && (
            <div className="space-y-1 pt-1">
              ...
              <p
                aria-live="polite"
                className={`text-xs ${countWords(customCategory) >= CUSTOM_CATEGORY_MAX_WORDS ? 'text-warning font-medium' : 'text-muted-foreground'}`}
              >
                {countWords(customCategory)}/{CUSTOM_CATEGORY_MAX_WORDS} palavras
                {countWords(customCategory) >= CUSTOM_CATEGORY_MAX_WORDS
                  ? ' — limite atingido'
                  : ''}
              </p>
            </div>
