...
import { Loader2, Info, Copy, UserPlus } from 'lucide-react';
...
export default function SettingsScreen() {
  ...
  const handleCreateEmployee = async () => {
    ...
    try {
      const result = await auth.adminCreateUser(employeeName.trim(), employeeEmail.trim(), 'manager');
    ...
    <div className="space-y-6 max-w-xl">
      ...
      <div className="space-y-4 rounded-lg border border-border bg-card p-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">Cria uma conta de manager com senha temporária — copie e envie pro funcionário por fora (WhatsApp, etc.), não é enviada por e-mail.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="space-y-2">
            <Label>Nome</Label>
            <Input value={employeeName} onChange={e => setEmployeeName(e.target.value)} placeholder="Nome completo" />
          </div>
          <div className="space-y-2">
            <Label>E-mail</Label>
            <Input type="email" value={employeeEmail} onChange={e => setEmployeeEmail(e.target.value)} placeholder="funcionario@empresa.com" />
          </div>
        </div>
        <Button onClick={handleCreateEmployee} disabled={creatingEmployee} className="gap-2">
          {creatingEmployee ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserPlus className="h-4 w-4" />} Criar funcionário
        </Button>
        ...
        {createdCredential && (
          <Alert>
            ...
            <AlertDescription className="space-y-2">
              <p><span className="text-muted-foreground">E-mail:</span> {createdCredential.email}</p>
              <p><span className="text-muted-foreground">Senha temporária:</span> <code className="font-mono">{createdCredential.temporaryPassword}</code></p>
              <Button type="button" size="sm" variant="outline" className="gap-1.5" onClick={handleCopyCredential}>
        ...
        <div className="flex gap-3 pt-1 border-t border-border">
          ...
          <p className="text-xs text-muted-foreground pt-2.5">Isso funciona neste ambiente local de testes. Em produção depende do tenant-gateway real oferecer o mesmo endpoint — até lá, promoção manual via console/banco é a alternativa (mesma limitação documentada na Story 6.3).</p>
        </div>
      </div>
    </div>
  );
