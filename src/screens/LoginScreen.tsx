...
import { ArrowLeft, Lock, Mail, Loader2, User, Phone } from 'lucide-react';
...
export default function LoginScreen() {
  ...
  const backToLogin = (
    <Button type="button" variant="ghost" size="sm" className="gap-1.5 -ml-2" onClick={() => setView('login')}>
          ...
          {view === 'admin-setup' && (
            <>
              ...
              <form className="space-y-4" onSubmit={handleSignup}>
                ...
                <p className="text-xs text-muted-foreground">Só funciona pro primeiro acesso do tenant — ele vira administrador automaticamente. Depois disso, novos funcionários são criados pelo administrador em Configurações.</p>
                <Button type="submit" className="w-full" disabled={loading}>
                  {loading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Criar conta de administrador
                </Button>
              </form>
            </>
          )}
